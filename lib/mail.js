...
const BRAND = { bg:"#0A0F22", deep:"#04060E", panel:"#10162E", edge:"#27325A",
                gold:"#E9B44C", bright:"#FFD98A", ink:"#EAEDF7", muted:"#97A0BF" };

const shell = inner => `<!doctype html><html><body style="margin:0;padding:0;background:${BRAND.deep};">
...
${r.notes ? `<tr><td style="padding:16px 44px 4px;">
...
async function resend(){
  ...
  if (!key){ console.error("[mail] RESEND_API_KEY unset — skipping email"); return null; }
...
export async function sendReservationEmails(r, meta){
  ...
  const from = process.env.FROM_EMAIL || "Meraki <onboarding@resend.dev>";
  const admin = process.env.ADMIN_EMAIL || "your-inbox@domain.com";
  const jobs = [
    rs.emails.send({ from, to: admin, reply_to: r.email,
      subject: `✦ New voyage request — ${r.name} · ${r.date}`,
      html: adminEmailHtml(r, meta) }),
    rs.emails.send({ from, to: r.email,
      subject: "✦ Your Meraki voyage is on our star charts",
      html: clientEmailHtml(r) })
  ];
  const [a, c] = await Promise.allSettled(jobs);
  for (const [label, res] of [["admin", a], ["client", c]])
    if (res.status === "rejected" || res.value?.error)
      console.error(`[mail] ${label} send failed:`, res.reason?.message || res.value?.error);
  return {
