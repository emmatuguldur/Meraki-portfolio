...
async function readBody(req){
  ...
  for await (const chunk of req){
    raw += chunk;
    if (raw.length > MAX_BODY) throw Object.assign(new Error("payload too large"), { code: 413 });
  }
...
export default async function handler(req, res){
  ...
  if (req.method !== "POST")
    return json(res, 405, { ok: false, error: "Method not allowed. POST only." });
  ...
  try { body = await readBody(req); }
    ...
    return json(res, e.code === 413 ? 413 : 400,
      { ok: false, error: e.code === 413 ? "Payload too large." : "Body must be valid JSON." });
  }
  ...
  const db = await saveReservation(data, meta);            // fail-soft: never blocks mail
  const mail = await sendReservationEmails(data, meta);

  if (!db.saved && !mail.admin && !mail.client)
    return json(res, 502, { ok: false,
      error: "Mission control is unreachable right now — please try again shortly." });
